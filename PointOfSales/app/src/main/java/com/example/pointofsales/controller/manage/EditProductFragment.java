...
public class EditProductFragment extends ProductFormFragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        mOriProduct = mProductViewModel.getProducts().getValue().getProductByIndex(getArguments().getInt("product_index"));
        mProductViewModel = ViewModelProviders.of(getActivity()).get(ProductViewModel.class);
        setData(mOriProduct);
    }
    ...
    public void submit() {
        ...
        product.setId(mOriProduct.getId());
        Map<String, Object> map = ProductRepository.productToMap(product);
        mLoadingScreenHelper.start();

        if (product.getName().equals(mOriProduct.getName()) || !mProductViewModel.checkIfNameExists(product.getName())) {
            ProductRepository.getInstance().updateDatabase(map, "0", new OnSuccessListener() {
                @Override
